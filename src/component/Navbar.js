...
class Navbar extends React.Component{
	  ...
	  constructor(props) {
        super(props);
        this.state = {visible: false, name:"Name....", hist:""};
		...
		return(
			...
			<img  src={prof}  class="profile" />
			 <Dialog header="Enter your name: " visible={this.state.visible} style={{width: '50vw'}} footer={footer} onHide={this.onHide} maximizable>
             	<input type="text" onChange={(e) => this.setState({hist: e.target.value})} />
             </Dialog>
			<div class="title" onClick={this.onClick}><strong>{this.state.name} </strong></div>			      
				<ul>
